...
public class AnalyzeBinary
{
    ...
    public static void main(String[] args)
    {
        ...
        {
            output.println();
            output.println("use: AnalyzeText \"source file\"");
        }
        ...
        if (analysis != null)
        {
            ...
            if (table != null)
            {
                ...
                for (int i = 0; i < tableHeaders; i++)
                    headerFields[i] = table.getHeaders()[i].length() + 2;
            }
...
/*
            for (int i = 0; i < tableHeaders; i++)
                output.print(StringFormat.padText(table.getHeaders()[i], headerFields[i], StringFormat.Alignment.left, ' '));
*/
            output.println();
            ...
            for (Map.Entry<Character, Double> entry : analysis.getCaseInsensitiveSet().descendingByValue().entrySet())
            {
                ...
                /*
                for (int i = 0; i < tableHeaders; i++)
                    output.print(StringFormat.padText(StringFormat.percent(Math.abs(entry.getValue() - table.get(table.getHeaders()[i], entry.getKey()))), headerFields[i], StringFormat.Alignment.left, ' '));
                */
                output.println();
            }
            ...
            if (table != null)
            {
                ...
                for (int i = 0; i < tableHeaders; i++)
                    output.print(StringFormat.padText(StringFormat.percent(variation.get(table.getHeaders()[i])), headerFields[i], StringFormat.Alignment.left, ' '));
